...
#define ADRES_2 1        // Adres uk³adu odbiorczego nr 2
#define KOMENDA_2 2      // Komendy przypisane do okreœlonych przycisków
#define KOMENDA_3 3      // KOMENDA_2 przydzielona jest do przycisku 2,
...
int main(void) {
	...
	while(1) {
		...
		if(flaga_dostepu) {	/* jeœli odebrano prawid³owe kody z pilota */
			...
			if( (adres == ADRES_2) && (komenda == KOMENDA_2) ) {              // Jeœli adres jest równy ADRES_2
				PORTB ^= (1<<PB0);                                            // to zmieñ stan na pinie PORTB.0

			}
			else if( (adres == ADRES_2) && (komenda == KOMENDA_3) ) {         // Jeœli adres jest równy ADRES_2
				                                                              // i komenda jest równa KOMENDA_3
				PORTB ^= (1<<PB0);                                            // to zmieñ stan na pinie PORTB.0
